...
enum class HttpStatus(val value: Int, val series: Series, val reasonPhrase: String) {
    ...
    fun is1xxInformational() = series == Series.INFORMATIONAL
    fun is2xxSuccessful() = series == Series.SUCCESSFUL
    fun is3xxRedirection() = series == Series.REDIRECTION
    fun is4xxClientError() = series == Series.CLIENT_ERROR
    ...
    companion object {
        ...
        fun resolve(statusCode: Int): HttpStatus? =
            VALUES.find { it.value == statusCode }
    }
    ...
    enum class Series(val value: Int) {
        ...
        companion object {
            fun resolve(statusCode: Int): Series? = entries.find { it.value == statusCode / 100 }
        }
    }
}
